...
public class Test1 {
    ...
    public static void plot2D(NondominatedPopulation approximationSet, int count){
        Plot plt = new Plot();
        plt.add("NSGA-II", approximationSet);
        String currentPath = System.getProperty("user.dir");
        String folderPath = currentPath + "/NSGA-II-2d-pics";
...
//        }
    ...
    public static String createFolder(){
        String currentPath = System.getProperty("user.dir");
        String folderPath = currentPath + "/NSGA-II-solutions";
        File folder = new File(folderPath);
        if (!folder.exists()) {
            folder.mkdir();
        }
    ...
    public static double runNSGAII(Problem problem, int seed, int probNum, int count, String refPath) throws IOException {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int i=0; i<probList.size(); i++)
        {
            for(int j=0; j<seeds.length; j++)
            {
                ...
                for(int k=0; k<10; k++)
                {
                    hypervolumes[k] = runNSGAII(probList.get(i),seeds[j],i,count, referencePaths[i]);
                    count++;
//                hypervolumes[0] = runNSGAII(probList.get(0),seeds[0]);
//                System.out.println(hypervolumes[0]);
                }
